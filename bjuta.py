...
async def logUserBot():
    load_dotenv()
    api_id = int(23894588)
    api_hash = "b0d854757e36ba5efc2fda3768384702"
    phone_number = "51936343280"
    ...
    await client.send_message("@spamm9024", f'<b>Bot encendido</b>', parse_mode="HTML")
    spammer_group = int("-4216023648")
    ...
    while True:
        ...
        try:
            await client.send_message("@spamm9024", f"<b>CANTIDAD DE MENSAJES CONSEGUIDOS PARA PUBLICAR</b> <code>{len(messages_list)-1}</code>",parse_mode="HTML")
        except:
            pass
                ...
                if i['group_name'] not in ["spam2024","RESPALDO🇵🇪BINS PERU","➳𝒀𝑨𝑷𝑬 𝑫𝑬 𝑬𝑺𝑻𝑨𝑭𝑨𝑫𝑶𝑹𝑬𝑺 ✧","QUEMANDO ESTAFADORES","𝐏𝐄𝐑Ú 𝐀𝐘𝐔𝐃𝐀","Referencias Elmer 💸","🎭 CANAL MUNDO STREAMING PERÚ 🇵🇪","TU MARKETPLACE"]:
                    ...
                    for message_spam in messages_list:
                        ...
                        try:
                            ...
                            await client.send_message("@spamm9024", f'<b>Error enviando mensajes a {i["group_id"]}</b> - <code>{i["group_name"]}<code>\nCausa:{error}',parse_mode="HTML")
                            resultado = False
                        if resultado:
                            await client.send_message("@spamm9024", f'<b>Mensaje enviado a {i["group_id"]}</b> - <code>{i["group_name"]}</code>',parse_mode="HTML")  
                        await asyncio.sleep(60)
                        if j==2: break
            await client.send_message("@spamm9024", f'<b>RONDA ACABADA</b>', parse_mode="HTML")
